...
class Items extends Secure_Controller
{
    ...
    /**
     * AJAX function. Rotates an item's picture 90° in the given direction.
     * Useful when a captured photo comes out sideways. POST 'direction':
     * 'cw' (right) or 'ccw' (left).
     * @param int $item_id
     * @return void
     * @noinspection PhpUnused
     */
    public function postRotateItemImage(int $item_id): void
    {
        $this->response->setContentType('application/json');
        ...
        if ($degrees === 0) {
            echo json_encode(['success' => false, 'message' => lang('Items.rotate_fail')]);
            return;
        }

        $pic_filename = $this->item->get_info($item_id)->pic_filename ?? null;
        ...
        $base_path = FCPATH . 'uploads/item_pics/' . pathinfo($pic_filename, PATHINFO_FILENAME);
        $image_path = empty($file_extension)
            ? (glob("$base_path.*")[0] ?? null)
            : "$base_path.$file_extension";

        if (!$image_path || !is_file($image_path)) {
            echo json_encode(['success' => false, 'message' => lang('Items.image_not_found')]);
            return;
        }

        $src = match (strtolower(pathinfo($image_path, PATHINFO_EXTENSION))) {
            'png'  => @imagecreatefrompng($image_path),
            'gif'  => @imagecreatefromgif($image_path),
            'webp' => @imagecreatefromwebp($image_path),
            default => @imagecreatefromjpeg($image_path),
        };
        ...
        $out_ext = strtolower(pathinfo($image_path, PATHINFO_EXTENSION));
        if ($out_ext === 'png') {
            imagealphablending($rotated, false);
            imagesavealpha($rotated, true);
        }

        // Write to a temp file then rename over the original: works even when
        // the file is owned by another user (e.g. inventory-service uploads as
        // ismael:ismael), because Apache can write the directory but not the
        // file itself.
        $tmp_path = $image_path . '.rot';
        $saved = match ($out_ext) {
            'png'  => imagepng($rotated, $tmp_path, 9),
            'gif'  => imagegif($rotated, $tmp_path),
            'webp' => imagewebp($rotated, $tmp_path, 90),
            default => imagejpeg($rotated, $tmp_path, 90),
        };
        imagedestroy($rotated);

        if (!$saved || !is_file($tmp_path)) {
            @unlink($tmp_path);
            echo json_encode(['success' => false, 'message' => lang('Items.rotate_fail')]);
            return;
        }
        ...
        // Thumbnail generated by getPicThumb would be stale
        $thumb_path = $base_path . "_thumb.$out_ext";
        ...
        echo json_encode([
            'success' => true,
            'url'     => base_url('uploads/item_pics/' . basename($image_path)),
        ]);
    }
